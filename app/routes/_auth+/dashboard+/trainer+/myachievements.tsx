...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const statsResult = await reportDB.getUserTotalStats(currentUser.id, (context as any).cloudflare.env.DATABASE_URL);
    
    console.log("My Stats result:", statsResult);
    
    // Get user reports with skills
    const reportsResult = await reportDB.getAllReports((context as any).cloudflare.env.DATABASE_URL);
    
    console.log("My Reports result:", reportsResult);
    
    const userReports = reportsResult.success && reportsResult.data ? 
      reportsResult.data.filter((report: any) => report.userId === currentUser.id) : [];
    
    console.log("My User reports:", userReports);
    ...
    throw new Response("Failed to load achievements data", { status: 500 });
  }
}
...
export const MyAchievements = (): JSX.Element => {
  ...
  const metricCards = [
    ...
    {
      label: "تقرير",
      value: statistics?.reportCount?.toString() || "0",
      unit: "",
                    ...
                    <div className="relative w-[120px] h-[120px]">
                      <Doughnut
                        data={createCircleChartData(
                          statistics ? Math.min(100, Math.max(5, (statistics?.reportCount || 0) * 5)) : 25
                        )}
